...
def train_model(
    X_train: pd.DataFrame, y_train: np.array, n_estimators: int
...
def predict(
    model: GradientBoostingClassifier, X_test: pd.DataFrame, y_test: np.array
) -> float:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train lgradient boosting")
    parser.add_argument("test_size", type=float, help="Data split test size")
    parser.add_argument("n_estimators", type=int, help="N estimators")
